...
namespace Shared
{
    ...
    public class DBConvert
    {
        ...
        /// <param name="value"></param>  Định dạng "hh:mm:ss"
        /// <param name="format"></param>
        /// <returns></returns>
        public static DateTime ParseDateTimeBigger24(DateTime date, string value, string format)
        {
            DateTime result;
            try
            {
                ...
                info.ShortDatePattern = format;
                info.FullDateTimePattern = format;
                result = date; // gán bằng mốc ngày
                bool isCrossDay = false;
                int hour = 0, minute = 0, second = 0;

                String[] arrValue = value.Split(':');
                if (arrValue.Length > 0)
                    hour = DBConvert.ParseInt(arrValue[0]);
                if (arrValue.Length > 1)
                    minute = DBConvert.ParseInt(arrValue[1]);
                if (arrValue.Length > 2)
                    second = DBConvert.ParseInt(arrValue[2]);
                // Lớn qua 24h
                if (hour >= 24)
                {
                    int addDay = hour / 24;
                    isCrossDay = true;
                    result = date.AddDays(addDay);
                }
                result = new DateTime(result.Year, result.Month, result.Day, isCrossDay ? hour - 24 : hour, minute, second);
            }
            catch { result = DATETIME_MIN_VALUE; }
            return result;
        }

        public static string ParseTimeCrossDay(DateTime date, DateTime calculate)
        {
            string result;
            try
            {
                TimeSpan ts = calculate - date;
